...
def get_status(limit: int = 50):
    """Status endpoint for ops/debug.

    MVP-friendly but actionable:
    - counts
    - missing lists (limited)
    - per-provider image coverage
    ...
    with Session(engine) as session:
        ...
        recent_failed = session.exec(
            select(Paper.external_id, PaperImage.provider, PaperImage.order_idx, PaperImage.error, PaperImage.updated_at)
        ...
        recent_paper_failed = session.exec(
            select(Paper.external_id, PaperEvent.stage, PaperEvent.error, PaperEvent.log_path, PaperEvent.created_at)
    ...
    return {
        ...
        "paper_images": {
            ...
            "recent_failed": [
                ...
                for (eid, prov, oidx, err, t) in recent_failed
            ],
        },
        "paper_events": {
